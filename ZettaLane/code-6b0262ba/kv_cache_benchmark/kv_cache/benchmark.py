...
class IntegratedBenchmark:
    ...
    def run(self) -> Dict:
        ...
        if not self.use_burst_trace and not self.use_dataset:
            ...
            print(f"  Mean: {np.mean(context_lengths):.0f} tokens ({np.mean(context_lengths) * bytes_per_token_per_rank / 1024**2:.2f}MiB)")
    ...
    def _print_summary(self, summary: Dict):
        ...
        # Hoisted from inside the PR #545 docstring trap so cache_stats is
        # available to later print statements (Cache Hit Rate etc.).
