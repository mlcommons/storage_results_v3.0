...
def generate_mpi_prefix_cmd(
    ...
    # Local patch: drop --npernode unconditionally. The "-n N -host host:N"
    # construction above already specifies the per-host rank layout, and
    # OpenMPI 5.x deprecates --npernode by auto-translating it to
    # --map-by ppr:N:node, which conflicts with the explicit --map-by socket
    # default emitted below.
