...
class TrainingBenchmark(DLIOBenchmark):
    ...
    def __init__(self, args, **kwargs):
        ...
        if self.args.command != "datasize" and self.args.data_dir:
            ...
            if self.args.command == "datagen" and self.args.mode != "whatif":
                ...
                assert_data_dir_hierarchy_absent(
                    self._dataset_root_location(), self.args.model
                )
    ...
    def _dataset_root_location(self):
        """Return the real local path or object URI behind ``--data-dir``."""
        data_dir = self.args.data_dir
        if urlparse(data_dir).scheme:
            return data_dir
        storage_type = self.params_dict.get('storage.storage_type', 'local')
        if storage_type == 'local':
            return data_dir
        scheme = self.params_dict['storage.storage_options.uri_scheme']
        root = self.params_dict['storage.storage_root']
        if scheme == 'direct':
            return f'direct://{root.rstrip("/")}'
        return f'{scheme}://{root.strip("/")}/{data_dir.lstrip("/")}'
    ...
    def _post_datagen_actions(self) -> bool:
        ...
        """
        # main.py writes metadata only after _run() returns, but this leaf
        # gate executes inside _run(). Write it now so a successful datagen
        # can satisfy the submission check; main.py later refreshes the file.
        self.write_metadata()
        ...
        manifest_path = write_datagen_manifest(
            data_dir=self._dataset_root_location(),
